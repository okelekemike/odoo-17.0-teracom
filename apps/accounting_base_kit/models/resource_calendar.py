...
class ResourceCalendar(models.Model):
    ...
    hr_department = fields.Many2one('hr.department', string="Department", required=True,
                                    help="Department", check_company=True, default=lambda self: self.env.company.id)
    sequence = fields.Integer(string="Sequence", required=True, default=1, help="Sequence")
    attendance_ids = fields.One2many('resource.calendar.attendance', 'calendar_id', 'Workings Time',
    ...
    def _interval_new(self, start_datetime, end_datetime, kw=None):
        """ Create a new interval object with start and end datetime, and optional data.
    ...
    def _get_day_work_intervals(self, day_date, start_time=None, end_time=None, compute_leaves=False, resource_id=None):
        """ Calculate working intervals for a given day, optionally filtering by start/end times and leaves.
        :return: A list of interval objects representing working periods within the specified constraints.
        """
        self.ensure_one()
