...
class MasterSearch(models.Model):
    ...
    _name = 'master.search'
    _description = "Master search model"
    _rec_name = 'name'
    _order = "create_date desc"

    name = fields.Char(string="Name", default=lambda self: _('Search'), help='Searched phrase expression.')
    ...
    master_search_ids = fields.Many2many('master.search', 'master_search_self_rel',
    ...
    transaction_ids = fields.Many2many('stock.picking',
                                       ...
                                       'search_id', 'company_id',
                                       string="Inventory", help='To fetch datas of inventory search')
    ...
    sale_ids = fields.Many2many('sale.order', 'master_search_sale_details_rel',
                                'search_id', 'company_id', string="Sale",  help='To fetch datas of sale search')
    purchase_ids = fields.Many2many('purchase.order',
                                    'master_search_purchase_details_rel',
                                    'search_id', 'company_id', string="Sale",
                                    help='To fetch datas of purchase search')
    account_ids = fields.Many2many('account.move',
                                   'master_search_account_details_rel',
                                   'search_id', 'company_id', string="Account",
                                   help='To fetch datas of account search')

    @api.depends('search_string')
    def _get_recent_searches(self):
        """ Get recent searches """
        try:
            current_id = self.id if isinstance(self.id, int) else self._origin.id
        except:
            current_id = False
            pass
        empty_search = self.env['master.search'].search(
            [('search_string', 'in', ['', False]),
             ('id', 'not in', [current_id, False] if current_id else [False])])
        if empty_search:
            empty_search.unlink()
    ...
    def action_unlink_search(self):
        """ Unlink search """
        self.unlink()
        action = self.env.ref('accounting_base_kit.master_search_action').read()[0]
        return action
