...
class TurnoverReport(models.TransientModel):
    ...
    def action_pdf_report_generate(self):
        ...
        return (self.env.ref(
            'accounting_base_kit.inventory_turnover_report').
    ...
    def action_xlsx_report_generate(self):
        ...
        return {
            ...
            'data': {'model': 'turnover.report',
                     ...
                     'output_format': 'xlsx',
                     'report_name': 'Inventory Turnover Analysis',
                     },
            'report_type': 'xlsx',
        }
    ...
    def get_xlsx_report(self, data, response, report_name):
        ...
        sheet.merge_range('A3:G1', report_name, head)
        row = 6
        column = 0
        if data['start_date']:
        ...
        for record in record_dict.values():
            ...
            if ((not self.start_date and not self.end_date) or
                    (self.start_date and self.end_date and record_date and
                     self.start_date <= self.end_date and self.start_date <= record_date <= self.end_date) or
                    (self.start_date and not self.end_date and record_date and record_date >= self.start_date) or
                    (not self.start_date and self.end_date and record_date and record_date <= self.end_date)):
