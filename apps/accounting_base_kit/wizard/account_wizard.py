...
class AccountWizard(models.TransientModel):
    ...
    def generate_pdf_report(self):
        ...
        return self.env.ref(
            'accounting_base_kit.pdf_report_action').report_action(
            self,
            data=data)
    ...
    def get_xlsx_report(self, data, response):
        ...
        if data['levels'] == 'summary':
            ...
            extract(YEAR from am.date) as year_part, sum(aml.debit) AS total_debit, sum(aml.credit) AS total_credit, 
            ...
            aml.credit) AS total_credit,sum(aml.balance) AS total_balance FROM (
                ...
                data['date_to']) + """' ) am
                    LEFT JOIN account_move_line aml ON aml.move_id = am.id
                    LEFT JOIN account_account aa ON aa.id = aml.account_id
                         """ + state + """ GROUP BY aa.id,aa.name ORDER BY aa.id,aa.name """
            cr = self._cr
            cr.execute(query2)
            fetched_data = cr.dictfetchall()
            ...
            query1 = """SELECT aa.id,aa.name,aa.code, sum(aml.debit) AS total_debit,
                ...
                data['date_to']) + """' ) am
                         LEFT JOIN account_move_line aml ON aml.move_id = am.id
                         LEFT JOIN account_account aa ON aa.id = aml.account_id
                         """ + state + """GROUP BY aa.id,aa.name, aa.code ORDER BY aa.id,aa.name, aa.code """
            cr = self._cr
            cr.execute(query1)
            fetched_data = cr.dictfetchall()
