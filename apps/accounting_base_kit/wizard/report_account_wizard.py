...
class ReportAccountWizard(models.AbstractModel):
    ...
    _name = "report.accounting_base_kit.cash_flow_pdf_report"
    ...
    def _get_report_values(self, docids, data=None):
        ...
        if data['levels'] == 'summary':
            ...
            query3 = """SELECT to_char(am.date, 'MM') as month_no, to_char(am.date, 'Month') as month_part, 
            extract(YEAR from am.date) as year_part, sum(aml.debit) AS total_debit, sum(
            ...
            SELECT am.date, am.id, am.state FROM account_move as am
                     ...
                     LEFT JOIN account_move_line aml ON aml.move_id = am.id
                     LEFT JOIN account_account aa ON aa.id = aml.account_id
                     """ + state + """ GROUP BY month_no,month_part,year_part ORDER BY month_no,year_part """
            cr = self._cr
            cr.execute(query3)
            ...
            aml.credit) AS total_credit,sum(aml.balance) AS total_balance FROM(
                 ...
                 LEFT JOIN account_move_line aml ON aml.move_id = am.id
                 LEFT JOIN account_account aa ON aa.id = aml.account_id
                 """ + state + """ GROUP BY aa.name ORDER BY aa.name """
            cr = self._cr
            cr.execute(query2)
            fetched_data = cr.dictfetchall()
            ...
            query1 = """SELECT aa.name ->'%s' as name,aa.code, sum(
                    aml.debit) AS total_debit, sum(aml.credit) AS total_credit,
                         ...
                         LEFT JOIN account_move_line aml ON aml.move_id = am.id
                         LEFT JOIN account_account aa ON aa.id = aml.account_id
                         """ + state + """ GROUP BY aa.name, aa.code ORDER BY aa.name, aa.code """
            cr = self._cr
            cr.execute(query1)
            fetched_data = cr.dictfetchall()
            ...
            aml.debit) AS total_debit,
                ...
                data['date_to']) + """'  """ + state + """) am
                         LEFT JOIN account_move_line aml ON aml.move_id = am.id
                         LEFT JOIN account_account aa ON aa.id = aml.account_id
                         GROUP BY aa.name, aa.code ORDER BY aa.name, aa.code """
            cr = self._cr
            cr.execute(sql)
    ...
    def _get_lines(self, account, data):
        ...
        aml.debit) AS total_debit, sum(aml.credit) AS total_credit FROM (
             SELECT am.* FROM account_move as am
             LEFT JOIN account_move_line aml ON aml.move_id = am.id
             LEFT JOIN account_account aa ON aa.id = aml.account_id
             WHERE am.date BETWEEN '""" %(user_lang) + str(
