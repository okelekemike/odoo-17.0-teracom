...
class UserPortal(CustomerPortal):
    """Creates the class Customer Portal to manage the controllers related to this module"""
    ...
    def portal_my_records(self, page=1, date_begin=None, date_end=None, sortby=None):
        """Method portal_my_records to return the employee verification records
        assigned for the current agency"""
        partner = request.env.user.partner_id
        employee_records = request.env['employee.verification'].sudo().search(
            ['&', ('state', '=', 'assign'), ('agency_id', '=', partner.id)])
        verification_count = request.env['employee.verification'].sudo().search_count(
            ['&', ('state', '=', 'assign'), ('agency_id', '=', partner.id)])
        pager = portal_pager(
            url="/my/quotes",
        ...
        return request.render("accounting_base_kit.portal_my_records", values)
    ...
    def portal_record_page(self, order=None):
        ...
        return request.render("accounting_base_kit.portal_record_page", values)
    ...
    def portal_order_report(self, **kw):
        ...
        if kw['description'] or kw.get('attachment', False):
            ...
            return request.render("accounting_base_kit.portal_record_completed", values)
        raise UserError(_("You need to Enter description or attach a file before submit."))
