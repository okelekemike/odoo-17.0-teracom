...
class StockCardView(models.TransientModel):
    ...
    reference_origin = fields.Char(compute="_name_get")
    ...
    def _name_get(self):
        for rec in self:
            ...
            if rec.picking_id:
                name = "{} ({}) {}".format(
                    name, rec.picking_id.origin, rec.picking_id.partner_id.name or "No Partner Listed"
                )
            rec.reference_origin = name
...
class StockCardReport(models.TransientModel):
    ...
    def print_report(self, report_type="qweb"):
        self.ensure_one()
        action = (
            self.env.ref("accounting_base_kit.action_stock_card_report_pdf")
        )
