...
patch(ListController.prototype, {
    /*
    * Retrieve the fields to be displayed in the list of export
    * */
    _RetrieveExportData: async function () {
        // Retrieve the fields to export
        const fields = this.props.archInfo.columns.filter((col) => (
                (col.optional === false || col.optional === "show") && col.invisible !== "True")).map(
                    (col) => this.props.fields[col.name]
        );
    ...
    */
    _onClickPDF: async function() {
        var self = this;
        // Retrieve the fields to export
        const retrieveExportFields = await this._RetrieveExportData();
        const fields = retrieveExportFields['fields'];
        const resIds = retrieveExportFields['res_ids'];
        const exportFields = fields.map((field) => ({
            name: field.name,
            label: field.label || field.string,
        }));
        ...
        jsonrpc('/get_data', {
            ...
            return self.model.action.doAction({
                ...
                report_name: 'accounting_base_kit.print_pdf_listview',
                report_file: "accounting_base_kit.print_pdf_listview",
                data: {
                    'length': length_field,
                    'record': data
                }
